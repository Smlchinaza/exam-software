...
const TeacherSignUp = () => {
  ...
  const { signupTeacher, logout } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await signupTeacher(formData.email, formData.password, formData.displayName);
          ...
          <form className="space-y-4 xs:space-y-6" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              <p className="mt-1 text-xs xs:text-sm text-gray-500">
                Your unique code will be sent to this email address.
              </p>
            </div>
