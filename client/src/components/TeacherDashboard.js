...
import { FaChalkboardTeacher, FaBook, FaClipboardList, FaUser, FaUsers, FaTasks, FaSignOutAlt, FaBars, FaTimes, FaQuestionCircle, FaListAlt, FaUserCircle, FaChartBar, FaGraduationCap, FaCheckCircle, FaTable, FaUserFriends, FaEye } from 'react-icons/fa';
      ...
      <header className="bg-gradient-to-r from-blue-700 to-indigo-600 shadow-md w-full md:hidden animate-fade-in">
        <div className="flex items-center justify-between px-4 py-4">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-extrabold text-white tracking-wide drop-shadow animate-slide-in">Teacher Dashboard</h1>
          </div>
          <div className="flex items-center gap-2">
            <button className="md:hidden text-white text-2xl focus:outline-none" onClick={() => setMobileNavOpen(!mobileNavOpen)} aria-label="Toggle navigation">
        ...
        <div className="fixed inset-0 z-40 flex md:hidden animate-fade-in">
          ...
          <div className="relative z-50 h-full w-64 bg-white shadow-lg p-4 animate-slide-in-left rounded-r-2xl">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-blue-700 text-left flex items-center gap-2"><FaChalkboardTeacher /> Navigation</h3>
              <button className="text-gray-700 focus:outline-none" onClick={() => setMobileNavOpen(false)} aria-label="Close navigation menu">
                <FaTimes className="w-6 h-6" />
              </button>
            </div>
            <ul className="space-y-2">
              <li className={`${activeNav==='dashboard' ? 'bg-blue-100 text-blue-700' : 'hover:bg-gray-50'} p-2 rounded flex items-center gap-2`}>
                <button onClick={() => {navigate('/teacher/dashboard'); setActiveNav('dashboard'); setMobileNavOpen(false);}} className="flex items-center gap-2 w-full text-left"><FaChartBar /> Dashboard</button>
              </li>
              <li className={`${activeNav==='question-bank' ? 'bg-blue-100 text-blue-700' : 'hover:bg-gray-50'} p-2 rounded flex items-center gap-2`}>
                <button onClick={() => {navigate('/teacher/question-bank'); setActiveNav('question-bank'); setMobileNavOpen(false);}} className="flex items-center gap-2 w-full text-left"><FaBook /> Question Bank</button>
              </li>
              <li className={`${activeNav==='create-exam' ? 'bg-blue-100 text-blue-700' : 'hover:bg-gray-50'} p-2 rounded flex items-center gap-2`}>
                <button onClick={() => {navigate('/teacher/create-exam'); setActiveNav('create-exam'); setMobileNavOpen(false);}} className="flex items-center gap-2 w-full text-left"><FaClipboardList /> Create Exam</button>
              </li>
              <li className={`${activeNav==='active-exams' ? 'bg-blue-100 text-blue-700' : 'hover:bg-gray-50'} p-2 rounded flex items-center gap-2`}>
                <button onClick={() => {navigate('/teacher/active-exams'); setActiveNav('active-exams'); setMobileNavOpen(false);}} className="flex items-center gap-2 w-full text-left"><FaListAlt /> Active Exams</button>
              </li>
      ...
      <div className="w-56 xs:w-64 bg-white shadow-lg hidden md:block animate-fade-in">
        <div className="p-4">
          <div className="flex items-center gap-2 mb-6">
            <img src={require('../assets/images/SpectraLogo.jpg')} alt="Logo" className="w-8 h-8 rounded-full shadow" />
            <span className="font-bold text-blue-700 text-lg">Teacher Portal</span>
          </div>
          <ul className="space-y-2">
            <li className={`${activeNav==='dashboard' ? 'bg-blue-100 text-blue-700' : 'hover:bg-gray-50'} p-2 rounded flex items-center gap-2`}>
              <button onClick={() => {navigate('/teacher/dashboard'); setActiveNav('dashboard');}} className="flex items-center gap-2 w-full text-left"><FaChartBar /> Dashboard</button>
            </li>
            <li className={`${activeNav==='question-bank' ? 'bg-blue-100 text-blue-700' : 'hover:bg-gray-50'} p-2 rounded flex items-center gap-2`}>
              <button onClick={() => {navigate('/teacher/question-bank'); setActiveNav('question-bank');}} className="flex items-center gap-2 w-full text-left"><FaBook /> Question Bank</button>
            </li>
            <li className={`${activeNav==='create-exam' ? 'bg-blue-100 text-blue-700' : 'hover:bg-gray-50'} p-2 rounded flex items-center gap-2`}>
              <button onClick={() => {navigate('/teacher/create-exam'); setActiveNav('create-exam');}} className="flex items-center gap-2 w-full text-left"><FaClipboardList /> Create Exam</button>
            </li>
            <li className={`${activeNav==='active-exams' ? 'bg-blue-100 text-blue-700' : 'hover:bg-gray-50'} p-2 rounded flex items-center gap-2`}>
              <button onClick={() => {navigate('/teacher/active-exams'); setActiveNav('active-exams');}} className="flex items-center gap-2 w-full text-left"><FaListAlt /> Active Exams</button>
            </li>
            <li className={`${activeNav==='results' ? 'bg-blue-100 text-blue-700' : 'hover:bg-gray-50'} p-2 rounded flex items-center gap-2`}>
        ...
        <div className="p-2 xs:p-4 md:p-8 animate-fade-in">
          ...
          <div className="overflow-x-auto animate-fade-in">
            <table className="min-w-full bg-white border border-gray-200 rounded-xl overflow-hidden text-sm shadow-md">
              <thead className="bg-gray-100">
                <tr>
                  <th className="px-4 py-2 text-left font-medium text-gray-600">Exam Name</th>
              ...
              <tbody>
                {recentExams.map((exam, idx) => (
                  <tr key={exam.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50 hover:bg-blue-50 transition'}>
                    <td className="px-4 py-3 text-left font-medium">{exam.name}</td>
                    ...
                    <td className="px-4 py-3 text-left flex gap-2">
                      ...
                      {exam.status === 'Active' ? (
                        <>
                          <button className="bg-blue-500 text-white px-3 py-1 rounded-lg text-xs hover:bg-blue-600 flex items-center gap-1 shadow" title="View">
                            <FaEye />
                          </button>
                          <button className="bg-gray-500 text-white px-3 py-1 rounded-lg text-xs hover:bg-gray-600 flex items-center gap-1 shadow" title="Edit">
                            <FaClipboardList />
                          </button>
                        </>
                        ...
                        <button className="bg-blue-500 text-white px-3 py-1 rounded-lg text-xs hover:bg-blue-600 flex items-center gap-1 shadow" title="Preview">
                          <FaEye />
                        </button>
                      )}
                    </td>
          ...
          <div className="mt-10 animate-fade-in">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h3 className="text-lg font-semibold text-gray-800">My Subjects & Students</h3>
            </div>
            {!mySubjects || mySubjects.length === 0 ? (
              <p className="text-gray-500 text-sm">No subjects assigned to you.</p>
            ) : (
              mySubjects.map(subj => (
                <details key={`${subj.name}|${subj.class}`} className="mb-6 bg-white rounded-xl shadow-lg border border-gray-100 animate-slide-in">
                  <summary className="cursor-pointer py-3 px-4 font-semibold text-blue-700 flex items-center gap-2 text-base hover:bg-blue-50 rounded-t-xl">
                    <FaBook className="text-blue-400" /> {subj.name} <span className="text-xs text-gray-500 ml-2">({subj.class})</span>
                  </summary>
                  <div className="p-4">
                    {!studentsBySubject[`${subj.name}|${subj.class}`] || studentsBySubject[`${subj.name}|${subj.class}`].length === 0 ? (
                      <p className="text-gray-500 text-sm ml-2">No students in this class.</p>
                    ) : (
                      <table className="min-w-full text-xs sm:text-sm border mb-2 rounded-lg overflow-hidden shadow animate-fade-in">
                        ...
                        <tbody>
                          {studentsBySubject[`${subj.name}|${subj.class}`].map(stu => (
                            <tr key={stu._id} className="even:bg-white odd:bg-gray-50 hover:bg-blue-50 transition">
