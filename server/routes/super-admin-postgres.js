 ...
 * Get all school admins with password management information (Super Admin only)
 */
...
router.post('/school-admins/:adminId/reset-password', authenticateJWT, requireSuperAdmin, async (req, res) => {
  ...
  try {
    const { adminId } = req.params;
    const { reason, notifyAdmin = true } = req.body;
    ...
    const adminRes = await client.query(
      `SELECT u.id, u.email, u.first_name, u.last_name, u.role, u.school_id,
              u.password_hash, s.name as school_name, u.school_id
       FROM users u
       LEFT JOIN schools s ON u.school_id = s.id
    ...
    // Import PasswordGenerator for this function
