...
DO $$ BEGIN -- Check and add missing columns with proper types and constraints
...
IF NOT EXISTS (
    ...
    FROM information_schema.columns
    WHERE table_name = 'schools'
        AND column_name = 'establishment_year'
) THEN
ALTER TABLE schools
...
DO $$ BEGIN -- Add CHECK constraints for type column
IF NOT EXISTS (
    SELECT 1
    FROM information_schema.check_constraints
    WHERE constraint_name = 'chk_schools_type'
) THEN
ALTER TABLE schools
ADD CONSTRAINT chk_schools_type CHECK (
        type IN (
            'primary',
            'secondary',
            'tertiary',
...
END IF;
-- Ensure status constraint includes all supported states
ALTER TABLE schools DROP CONSTRAINT IF EXISTS chk_schools_status;
ALTER TABLE schools
ADD CONSTRAINT chk_schools_status CHECK (
        status IN (
            'active',
            'inactive',
            'suspended',
...
IF NOT EXISTS (
    SELECT 1
    FROM information_schema.check_constraints
    WHERE constraint_name = 'chk_schools_accreditation'
) THEN
ALTER TABLE schools
ADD CONSTRAINT chk_schools_accreditation CHECK (
        accreditation_status IN ('pending', 'approved', 'rejected', 'expired')
    );
END IF;
...
IF NOT EXISTS (
    SELECT 1
    FROM information_schema.check_constraints
    WHERE constraint_name = 'chk_schools_establishment_year'
) THEN
ALTER TABLE schools
ADD CONSTRAINT chk_schools_establishment_year CHECK (
        establishment_year IS NULL
        OR (
            establishment_year >= 1800
            AND establishment_year <= EXTRACT(
                YEAR
                FROM NOW()
            )
        )
...
IF NOT EXISTS (
    SELECT 1
    FROM information_schema.check_constraints
    WHERE constraint_name = 'chk_schools_capacity'
) THEN
ALTER TABLE schools
ADD CONSTRAINT chk_schools_capacity CHECK (
        student_capacity IS NULL
        OR student_capacity > 0
    );
END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_schools_active_public ON schools(state_id)
WHERE status = 'active'
    AND is_public = true;
CREATE INDEX IF NOT EXISTS idx_schools_verified ON schools(state_id)
WHERE is_verified = true;
-- Create or update trigger function for updated_at
CREATE OR REPLACE FUNCTION update_schools_updated_at() RETURNS TRIGGER AS $$ BEGIN NEW.updated_at = NOW();
RETURN NEW;
END;
$$ language 'plpgsql';
...
-- Create trigger
CREATE TRIGGER trigger_update_schools_updated_at BEFORE
UPDATE ON schools FOR EACH ROW EXECUTE FUNCTION update_schools_updated_at();
...
WHERE s.status = 'active'
    ...
    AND (
        public_only IS NULL
        OR s.is_public = public_only
    )
    AND (
        state_filter IS NULL
        OR s.state_id = state_filter
    )
    AND (
        type_filter IS NULL
        OR s.type = type_filter
    )
    AND (
        search_query IS NULL
        OR to_tsvector(
            'english',
            s.name || ' ' || COALESCE(s.city, '') || ' ' || COALESCE(s.description, '')
